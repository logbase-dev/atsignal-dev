...
interface FAQ {
  ...
  views?: number; // 조회수 (기본값: 0, web 앱에서만 증가)
  createdAt?: Date; // 생성일시
  updatedAt?: Date; // 수정일시
...
- 검색 기능 (질문/답변 내용 검색)
- 해시태그 필터링 기능
- 정렬 기능 (level, isTop, createdAt, views 기준)
- FAQ 추가 버튼
- FAQ 수정/삭제 버튼
- 카테고리 관리 버튼
- 조회수 표시 (관리자 페이지에서는 조회수만 표시, 증가하지 않음)
...
│    카테고리: 일반 | Level: 1 | 조회수: 42 | #시작하기 #기본 #설치   │
├─────────────────────────────────────────────────────────┤
│ 질문: 어떻게 시작하나요?              [수정] [삭제]   │
│ 답변: 시작하려면...                                    │
│ 카테고리: 시작하기 | Level: 2                          │
...
export async function getFAQs(options?: {
  ...
  search?: string;
  orderBy?: "level" | "isTop" | "createdAt" | "views";
...
/ FAQ 조회수 증가 (web 앱에서만 사용)
export async function incrementFAQViews(id: string): Promise<void>;// ... existing code ...
...
## 5.3 조회수 증가 API (`/api/faqs/[id]/views`)

**기능:**

- web 앱에서 FAQ 답변을 조회할 때 조회수를 증가시킵니다.
- 관리자 페이지에서는 조회수가 증가하지 않습니다.

**구현 방식:**

- API Route: `POST /api/faqs/[id]/views`
- Firestore의 `increment` 연산을 사용하여 원자적으로 조회수 증가
- 중복 조회 방지: 세션 스토리지 또는 쿠키를 사용하여 같은 세션에서 중복 증가 방지 (선택사항)

**API 엔드포인트 설계:**
typescript
// POST /api/faqs/[id]/views
// Request: 없음 (URL 파라미터로 FAQ ID 전달)
// Response: { success: boolean, views: number }**주의사항:**

주의사항:

- 관리자 페이지(`/admin/faq`)에서 FAQ를 조회해도 조회수는 증가하지 않습니다.
- web 앱의 FAQ UI가 구현되면 해당 페이지에서만 조회수 증가 API를 호출합니다.
- 현재 web 앱에는 FAQ UI가 아직 구현되지 않았습니다. (향후 구현 예정)
...
1. **FAQ 평가 기능**: 도움이 되었는지 평가 (좋아요/싫어요)
2. **FAQ 관련 문서 링크**: 관련 페이지나 문서로 연결
3. **FAQ 검색 최적화**: Algolia 등 외부 검색 서비스 연동
4. **FAQ 버전 관리**: 답변 변경 이력 추적
5. **FAQ 자동 번역**: AI를 활용한 자동 번역 기능
6. **web 앱 FAQ UI 구현**: 사용자가 FAQ를 조회할 수 있는 페이지 구현 (조회수 증가 기능 포함)
...
- **조회수 기능:**
  - FAQ의 `views` 필드는 web 앱에서 사용자가 FAQ 답변을 조회할 때만 증가합니다.
  - 관리자 페이지에서 FAQ를 조회해도 조회수는 증가하지 않습니다.
  - 조회수는 Firestore의 `increment` 연산을 사용하여 원자적으로 증가시킵니다.
  - 현재 web 앱에는 FAQ UI가 아직 구현되지 않았습니다. (향후 구현 예정)
  - web 앱 FAQ UI 구현 시, FAQ 상세 페이지에서 조회수 증가 API를 호출합니다.
- **해시태그 기능:**
